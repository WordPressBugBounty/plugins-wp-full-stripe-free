<?php
    ...
    /** @var $modeFilter */
?>
<form name="wpfs-search-one-time-payments" action="<?php echo esc_url( $pageUrl ); ?>" method="post">
    <div class="wpfs-page-controls">
        <div class="wpfs-form-search wpfs-page-controls__control wpfs-page-controls__control--w320 js-form-search">
            <input class="wpfs-form-control wpfs-form-search__input" type="text" name="<?php echo MM_WPFS_Admin_Menu::PARAM_NAME_PAYMENTS_TEXT_FILTER ?>" value="<?php echo !empty( $textFilter ) ? esc_attr( $textFilter ) : "";  ?>" placeholder="<?php esc_html_e( 'Search...', 'wp-full-stripe-free'); ?>">
        ...
        <div class="wpfs-ui wpfs-form-select wpfs-page-controls__control wpfs-page-controls__control--w200">
            <select class="js-selectmenu js-one-time-payments-status-filter" name="<?php echo MM_WPFS_Admin_Menu::PARAM_NAME_PAYMENTS_STATUS_FILTER ?>" id="status" data-selectmenu-prefix="<?php _e( "Status: ", 'wp-full-stripe-free' ); ?>">
                <option value="all" <?php echo $statusFilter === MM_WPFS_Admin_Menu::PARAM_VALUE_PAYMENT_STATUS_ALL ? 'selected': ''; ?>><?php _e( 'All', 'wp-full-stripe-free'); ?></option>
                <?php
                foreach (MM_WPFS_Utils::getPaymentStatuses() as $paymentStatus ) {
                ?>
                <option value="<?php echo esc_attr( $paymentStatus ); ?>" <?php echo $statusFilter === $paymentStatus ? 'selected': ''; ?>><?php echo esc_html( MM_WPFS_Admin::getPaymentStatusLabel( $paymentStatus) ); ?></option>
                <?php } ?>
            </select>
        </div>
        <div class="wpfs-ui wpfs-form-select wpfs-page-controls__control wpfs-page-controls__control--w200">
            <select class="js-selectmenu js-one-time-payments-mode-filter" name="<?php echo MM_WPFS_Admin_Menu::PARAM_NAME_PAYMENTS_MODE_FILTER ?>" id="mode" data-selectmenu-prefix="<?php _e( "Mode: ", 'wp-full-stripe-free' ); ?>">
...
<script type="text/template" id="wpfs-side-pane-payment-details">
    <div class="wpfs-side-pane" data-db-id="<%- id %>" data-stripe-id="<%- paymentId %>">
        <div class="wpfs-side-pane__head">
            <div class="wpfs-side-pane__title"><?php _e( 'Payment details', 'wp-full-stripe-free' ); ?></div>
        ...
        <div class="wpfs-side-pane-item">
            ...
            <div class="wpfs-side-pane-item__actions">
                ...
                <div class="wpfs-tooltip-content" data-tooltip-id="capture-tooltip">
                    <div class="wpfs-info-tooltip"><?php _e( 'Capture', 'wp-full-stripe-free' ); ?></div>
                </div>
                <% } %>
                ...
                <div class="wpfs-tooltip-content" data-tooltip-id="remove-payment-tooltip">
                    <div class="wpfs-info-tooltip"><?php _e( 'Delete payment', 'wp-full-stripe-free' ); ?></div>
                </div>
            </div>
        </div>
        <div class="wpfs-side-pane-list">
            <div class="wpfs-side-pane-list__title"><?php _e( 'Payment', 'wp-full-stripe-free' ); ?></div>
            <ul class="wpfs-side-pane-list__list">
                <li class="wpfs-side-pane-list__item">
                    <?php _e( 'Id:', 'wp-full-stripe-free' ); ?> <a class="wpfs-btn wpfs-btn-link" href="<%- paymentUrl %>" target="_blank"><%- paymentId %></a>
                </li>
                <li class="wpfs-side-pane-list__item">
                ...
                <li class="wpfs-side-pane-list__item">
                    <?php _e( 'Status:', 'wp-full-stripe-free' ); ?> <span class="wpfs-side-pane-list__highlight"><%- localizedPaymentStatus %></span>
                </li>
                <li class="wpfs-side-pane-list__item">
